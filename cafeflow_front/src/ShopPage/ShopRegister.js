...
function ShopRegister() {
  ...
  useEffect(() => {
    const fetchCities = async (state) => {
      try {
        const response = await axios.get(
          `http://localhost:8080/api/locations/states/${state}/cities`
        );
    ...
    try {
      ...
      if (userInfo && userInfo.userType === "ADMIN") {
        ...
        const response = await axios.post(
          "http://localhost:8080/api/register-cafe",
          {
            name,
            address,
            description,
        ...
        const seatDataForPost = JSON.stringify(
          seats.map((seat) => ({
            seatHasPlug: seat.plug,
            seatSize: seat.size,
            seatNumber: seat.id,
            seatAngle:
              seat.orientation === "HORIZONTAL" ? "HORIZONTAL" : "VERTICAL",
            seatCoordinates: {
              x: seat.position.x,
              y: seat.position.y,
            },
